#!/usr/bin/env python3
"""Real-time DOA using pyroomacoustics SRP-PHAT (far-field).

This is mainly for comparing behaviour vs the GCC-PHAT baseline.
With only 2 microphones, DOA is not fully constrained in 2D/3D; interpret results as
"angle relative to the mic axis".
"""
...
def stft_frames(x: np.ndarray, nfft: int, hop: int, window: np.ndarray):
    ...
    return X.T.astype(np.complex64)  # (n_freq, n_frames)
...
def main():
    ...
    ap.add_argument("--frames", type=int, default=8, help="How many STFT frames to use")
    ...
    # mic geometry: mic1 at x=0, mic2 at x=d
    ...
    doa = pra.doa.SRP(L, fs=fs, nfft=int(args.nfft), c=SPEED_OF_SOUND, num_src=1, mode="far")
    ...
    with sd.InputStream(
        ...
        print("Running pyroomacoustics SRP-PHAT live. Ctrl+C to stop.")
        ...
        try:
            while True:
                ...
                X = np.stack([X1, X2], axis=0)  # (M, n_freq, n_frames)

                if X.shape[-1] < 2:
                    continue

                # pyroomacoustics API differs across versions; support both
